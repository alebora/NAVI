...
class Controller:
    ...
    def ready(self, wait_s=5.0):
        ...
        while True:
            ...
            if error is None:
                return s
            # Lost tracking will not heal by waiting; missing topics at startup might.
            hard = error.startswith("SLAM is not healthy") or error.startswith(
                "Invalid SLAM"
            )
        ...
        with self.lock:
            if self.mode == "awaiting_badge":
                ...
                if not ok:
                    ...
                    if (
                        ...
                        and not getattr(self, "_badge_reject_spoken", False)
                    ):
                ...
                if self.enabled:
                    ...
                    if pin is not None and pin_changed:
                        try:
                            ...
                            self.explore_targets = []
                            self.explore_visited = []
                            self.explore_hitch = health_error(s, hitch_ok=True) is not None
                            self.last_goal = None
                            self.last_pose = (
            ...
            if exploring:
                ...
                if pin is not None and np.linalg.norm(np.asarray(xy) - pin) < 0.6:
                    ...
                    pin = None
                    pin_changed = True
                    self.event("Reached explore pin; looking ahead for the next frontier.")
                if pin_changed:
                    self.last_goal = None
                    if pin is not None:
                        direct = nearest_clear_goal(
                            s["mapping.grid2d"], pin, self.robot.resolution
                        )
                        if direct is not None:
                            ...
                            self.event(
                                f"Explore pin ({pin[0]:.1f}, {pin[1]:.1f}) is not on free floor; "
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"Gemini Live model id (default {DEFAULT_LIVE_MODEL})",
    )
